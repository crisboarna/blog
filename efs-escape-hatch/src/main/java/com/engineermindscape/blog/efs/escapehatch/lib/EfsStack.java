...
public class EfsStack extends Stack {
    public EfsStack(final Construct scope, final String id, final BaseStackProps props) {
        ...
        SecurityGroup efsClusterSG = Utils.createSecurityGroup(
                this,
                projectName,
                stackEnv,
                vpc,
        ...
        this.getEfsDirectoryConfigurator(
                projectName,
                stackEnv,
                vpc,
        ...
        Utils.createSsmParameter(
                ...
                efsClusterSG.getSecurityGroupId()
        );
    }

    private void getEfsDirectoryConfigurator(
            String projectName, ENV stackEnv, IVpc vpc,
            AccessPoint efsLambdaAccessPoint, SecurityGroup efsClusterSG) {
        Function lambdaCreateDirs = Function.Builder
                ...
                .code(Code
                        .fromInline("const fs = require('fs');\n" +
                                ...
                                "    path.join(efsMountPath, 'rabbitmq/data')\n" +
                                "  ];\n" +
                                "\n" +
                                ...
                                "  };\n" +
                                "};"))
                .filesystem(software.amazon.awscdk.services.lambda.FileSystem.fromEfsAccessPoint(
                        efsLambdaAccessPoint,
                        "/mnt/efs")
                )
                .vpc(vpc)
        ...
        Provider provider = new Provider(
                this,
                projectName + "-EFS-Custom-Resource-Provider-" + stackEnv,
                ProviderProps
                        .builder()
                         .providerFunctionName(String.format("%s-EFS-Dir-Creator-Provider-%s", projectName, stackEnv))
                         .onEventHandler(lambdaCreateDirs)
                         .build());

        new CustomResource(this, projectName + "-EFS-Custom-Resource-" + stackEnv,
                CustomResourceProps
                        .builder()
